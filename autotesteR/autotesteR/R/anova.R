...
teste.anova <- function(..., titulo = "ANOVA/Tukey HSD", x = "Eixo x", y = "Eixo y", ajuda = FALSE) {
...
Uso:
  anova(grupo1, grupo2, ..., titulo = \"Titulo\", x = \"Eixo x\", y = \"Eixo y\")
...
")
  ...
  p_label <- if (p_anova < 0.001) {
    ...
    paste0("ANOVA: p = ", base::signif(p_anova, 3))
  }
  ...
  if (base::nrow(pares_signif) == 0) {
    base::message("Nenhuma comparacao post-hoc significativa (p < 0.05).")
  } else {
    ...
    base::message("Pares significativos (Tukey HSD):\n", base::paste(msg, collapse = "\n"))
  }
